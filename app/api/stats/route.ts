...
export async function GET(req: NextRequest) {
  try {
    ...
    const workoutStats = await executeWithRetry(
      () => sql`
    ...
    const habitStats = await executeWithRetry(
      () => sql`
        SELECT 
          COUNT(*) as total_logs,
    ...
    const foodStats = await executeWithRetry(
      () => sql`
        SELECT 
          COUNT(*) as total_logs,
    ...
    const weightTrend = await executeWithRetry(
      () => sql`
